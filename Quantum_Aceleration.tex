...
\begin{abstract}
Shor's algorithm for factoring large integers remains one of the most anticipated applications of quantum computing, yet its practical implementation is hindered by the enormous number of measurements required and the fragility of quantum coherence. Here we demonstrate that the modular ring $\mathbb{Z}/6\mathbb{Z}$—a structure governing the distribution of all prime numbers greater than three—enables a topological superselection framework that simultaneously addresses both limitations. By applying a phase $\phi=-0.940367$ derived from prime gap analysis, we achieve a $5.46\pm0.3$-fold reduction in required measurements for quantum factoring, while enhancing coherence through negative entropy gain of $2.53$ bits—exceeding the Shannon limit by $59.7\%$. For RSA-2048, this translates to reducing projected execution time from one year to 67 days on NISQ-era hardware. The phase $\phi$ emerges naturally from a unified equation governing prime gaps with $99.996\%$ precision, connecting number theory, geometry ($4\pi$), and information theory ($\ln2/4$) to the fine-structure constant $\alpha^{-1}=137.036$. This work demonstrates that arithmetic possesses an inherent quantum structure accessible for practical computation, and provides an immediately implementable enhancement to one of the most important quantum algorithms.
